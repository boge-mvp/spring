 ...
 * LogEvent 过滤，只要有一个过滤器拒绝 则立即返回
 * @see org.apache.logging.log4j.core.filter.CompositeFilter
 */
@Plugin(name = "AnyFilters", category = Node.CATEGORY, printObject = true)
@PerformanceSensitive("allocation")
public final class AnyFilter extends AbstractLifeCycle implements Iterable<Filter>, Filter {
    ...
    private AnyFilter(final Filter[] filters) {
        this.filters = filters == null ? Filter.EMPTY_ARRAY : filters;
    }

    public AnyFilter addFilter(final Filter filter) {
        if (filter == null) {
            // null does nothing
            return this;
        }
        ...
        if (filter instanceof AnyFilter) {
            final AnyFilter anyFilter = (AnyFilter) filter;
            final Filter[] copy = Arrays.copyOf(this.filters, this.filters.length + anyFilter.size());
            System.arraycopy(anyFilter.filters, 0, copy, this.filters.length, anyFilter.filters.length);
            return new AnyFilter(copy);
        }
    ...
    public AnyFilter removeFilter(final Filter filter) {
        if (filter == null) {
            // null does nothing
            return this;
        }
        ...
        if (filter instanceof AnyFilter) {
            for (final Filter currentFilter : ((AnyFilter) filter).filters) {
                filterList.remove(currentFilter);
            }
        } else {
            filterList.remove(filter);
        }
        return new AnyFilter(filterList.toArray(Filter.EMPTY_ARRAY));
    }
     ...
     */
    @PluginFactory
    public static AnyFilter createFilters(@PluginElement("Filters") final Filter[] filters) {
